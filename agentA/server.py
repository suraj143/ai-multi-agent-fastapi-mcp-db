...
MEMORY_FILE = Path("memory_agentA.json")
server = Server("AgentA")
model = get_model()
...
def fetch_schema1_customers():
    ...
    cur = conn.cursor()
    cur.execute("SET search_path TO customer1;")
...
def analyze_customer_data():
    data = fetch_schema1_customers()
    memory = load_memory()

    prompt = f"""
You are AgentA, responsible for schema1 customers.
Memory: {memory}
Data: {data}
