...
const PORT = Number(process.env.AGENTA_PORT || 8001);
const wss = new WebSocketServer({ port: PORT });

wss.on("connection", (ws) => {
  console.log("AgentA connected");
  ...
  ws.on("message", async (msg) => {
    ...
    try {
      if (tool === "fetch_schema1_customers") {
        const client = await pool.connect();
        try {
          await client.query("SET search_path TO customer1;");
      ...
      else if (tool === "analyze_customer_data") {
        const client = await pool.connect();
        try {
          await client.query("SET search_path TO customer1;");
          const res = await client.query("SELECT id, name FROM customers;");
          const prompt = `Analyze schema1 customers:\n${JSON.stringify(res.rows)}`;
...
console.log(`AgentA running on ws://0.0.0.0:${PORT}`);
